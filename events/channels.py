...
class Channels(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        temp = await self.bot.db.pending.find({'guild_id': member.guild.id, 'name': name})
        ...
        if after.channel and 'Créer' in after.channel.name and not member.bot and not temp:
            if cat := after.channel.category:
                ...
                try:
                    ...
                    await self.bot.db.pending.insert({'type': 'channel', 'guild_id': member.guild.id, 'name': name, 'txt_id': text.id})
                except:
        ...
        if before.channel:
            chan = await self.bot.db.pending.find({'guild_id': member.guild.id, 'name': before.channel.name})
            if chan and ((len(before.channel.members) <= 1 and member.guild.me in before.channel.members) or not len(before.channel.members)):
                ...
                await channel.delete()
                await self.bot.db.pending.delete({'guild_id': member.guild.id, 'name': before.channel.name})
